...
def inference_loop(config, ssl_model, val_dataloader):
    ...
    with torch.no_grad():
        for inputs, slide_id in tqdm(val_dataloader):
            ...
            inputs = inputs.to(DEVICE).to(config['dtype'])
            out = ssl_model(inputs)
            wsi_embed = out['wsi_emb']
...
def extract_wsi_embs_and_save(config, ssl_model, csv_path, features_path, save_fname):

    if hasattr(ssl_model, 'module'):
        MAX_PATCHES = 25000 if 'TransformerEncoder' in str(type(ssl_model.module.wsi_embedder)) else None
    else:
        MAX_PATCHES = 25000 if 'TransformerEncoder' in str(type(ssl_model.wsi_embedder)) else None
    ...
    results_dict, val_rank = inference_loop(config, ssl_model, test_dataloader)
